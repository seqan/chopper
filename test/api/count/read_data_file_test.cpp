...
TEST(read_data_file_with_clustering_test, file_open_error)
{
    ...
    EXPECT_THROW(chopper::count::read_data_file_with_clustering(config), std::runtime_error);
}
...
TEST(read_data_file_with_clustering_test, small_example)
{
...

    {
        ...
        auto filename_clusters = chopper::count::read_data_file_with_clustering(config);
