...
void initialize_argument_parser(sharg::parser & parser, chopper::count::configuration & config)
{
    ...
    parser.add_option(config.data_file,
                      sharg::config{.long_id = "input-file",
                                    .description = "The input must be a file containing paths to sequence data you "
                                                   "wish to estimate; one filepath per line. If your file contains "
                                                   "auxiliary information (e.g. species IDs), your file must be "
                                                   "tab-separated.",
                                    .required = true});
    ...
    parser.add_option(config.output_prefix,
                      sharg::config{.long_id = "output-prefix",
                                    .description = "The program creates a `[PREFIX].count` file that contains estimated"
                                                   " k-mer counts for each sequence file. These counts are used in "
                                                   "chopper layout to create an index layout file that can be used by "
                                                   "the app raptor to build an HIBF. Additionally, a directory "
                                                   "[PREFIX]_sketches is created that will contain one `.hll` file per "
                                                   "sequence file. The sketch files improve the layout computation. "
                                                   "Attention: Expects a prefix, thus file extensions are dropped and "
                                                   "if given a path (e.g. /tmp/), the default prefix " +
                                                   std::string{prefix::chopper} + " is appended.",
                                    .required = true});

    parser.add_option(config.threads,
                      sharg::config{.long_id = "threads",
                                    .description = "The number of threads to be used for parallel processing."});

    parser.add_option(config.column_index_to_cluster,
                      sharg::config{.long_id = "column-index",
                                    .description = "The column index by which to cluster. Clustering is not supported "
                                                   "at the moment",
                                    .hidden = true});
    ...
    parser.add_option(config.k,
                      sharg::config{.long_id = "kmer-size",
                                    .description = "The k-mer size influences the estimated counts. Choosing a k-mer "
                                                   "size that is too small for your data will result in files "
                                                   "appearing more similar than they really are. Likewise, a large "
                                                   "k-mer size might miss out on certain similarities. For DNA "
                                                   "sequences, a k-mer size between [16,32] has proven to work well."});

    parser.add_option(config.sketch_bits,
                      sharg::config{.long_id = "sketch-bits",
                                    .description = "The number of bits the HyperLogLog sketch should use to distribute "
                                                   "the values into bins.",
                                    .validator = sharg::arithmetic_range_validator{5, 32}});

    parser.add_flag(config.disable_sketch_output,
                    sharg::config{.long_id = "disable-sketch-output",
                                  .description = "Although the sketches will improve the layout, you might want to "
                                                 "disable writing the sketch files to disk. Doing so will save disk "
                                                 "space. However, you cannot use either --estimate-unions or "
                                                 "--rearrange-user-bins in chopper layout without the sketches. Note "
                                                 "that this option does not decrease run time as sketches have to be "
                                                 "computed either way."});
