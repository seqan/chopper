...
void set_up_subparser_layout(seqan3::argument_parser & parser, chopper::layout::configuration & config)
{
    ...
    parser.add_option(config.update_ubs,
                  '\0', "update-UBs",
...
void add_empty_bins(std::vector<size_t> & kmer_counts, std::vector<std::string> & filenames, double percentage)
{
    ...
    sort(kmer_counts_copy.begin(), kmer_counts_copy.end()); // data is not sorted, sort a copy
    int stepsize = std::round(1/percentage);
    auto original_size = kmer_counts.size();
    for (uint64_t idx=0; idx < kmer_counts_copy.size(); idx+stepsize){
...
int execute(seqan3::argument_parser & parser)
{
    ...
    add_empty_bins(data.kmer_counts, config.update_ubs); // add the empty bins
